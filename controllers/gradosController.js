...
const ver = async (req, res) => {
  try {
    ...
    // Alumnos matriculados en cualquier sección de este grado (solo con matrícula activa)
    const seccionIds = secciones.map(s => s._id);
    const matriculasRaw = await Matricula.find({
      seccion: { $in: seccionIds }, estado: 'activa',
    }).populate('alumno').populate('seccion');

    // Filtrar por si algún populate devuelve null y ordenar en memoria
