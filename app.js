...
// Contador de mensajes no leídos (disponible en todas las vistas)
app.use(async (req, res, next) => {
  if (req.session.usuarioId) {
    try {
      const { contarNoLeidos } = require('./controllers/mensajeriaController');
      res.locals.mensajesNoLeidos = await contarNoLeidos(req.session.usuarioId);
...
app.use('/portal-padre',      require('./routes/portalPadre'));
app.use('/portal-estudiante', require('./routes/portalEstudiante'));
...
app.get('/dashboard', async (req, res) => {
  ...
  if (rol === 'padre')      return res.redirect('/portal-padre');
  if (rol === 'estudiante') return res.redirect('/portal-estudiante');
